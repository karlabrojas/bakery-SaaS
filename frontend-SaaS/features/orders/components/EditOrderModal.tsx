...
export default function EditOrderModal({
  ...
  const { delivery, loadDelivery, update } = useDeliveries();
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (deliveryType === "DELIVERY" && deliveryData) {
        if (delivery) {
          await update(order.id, deliveryData);
        }
      }
      setExito(true);
      setTimeout(() => {
        ...
        <div className="w-full space-y-6 p-6">
          ...
          {soloLectura ? (
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 text-sm p-4 rounded-xl text-center font-medium">
              Este pedido no puede editarse porque está{" "}
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    {deliveryType === "DELIVERY" && (
                      <div className="mt-5">
                        <h3 className="font-bold text-[#472D20]">
